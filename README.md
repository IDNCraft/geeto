...
- **AI-Powered** — Branch names, commit messages, and release notes generated by Gemini, GitHub Copilot, OpenRouter, Groq, or OpenAI Codex
...
| **OpenAI Codex**   | Enter OpenAI API key (token auth) or use OAuth via `codex auth` CLI -- Codex models like `o3-mini`, `o4-mini`.  |
...
| `geeto --setup-groq`       | Configure Groq AI            |
| `geeto --setup-codex`      | Configure OpenAI Codex       |
...
| `codex.toml`           | `--setup-codex`           | OpenAI API key (Codex SDK models)                                         |
...
| `codex-model.json`        | `--setup-codex` / `--sync-models`   | Persisted Codex SDK model list                                           |
